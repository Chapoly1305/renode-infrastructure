...
namespace Antmicro.Renode.Peripherals.Miscellaneous.SiLabs
{
    public class SiLabs_KSU_1 : SiLabsPeripheral, SiLabs_IKeyStorage
    {
        ...
        public override void Reset()
        {
            ...
            statusRegister = 0;
            infoRegister = 0;
            rootControlRegister = 0;
            rootBaseAddressMetadata = 0;
            irkInterruptEnable = 0;
            irkInterruptFlags = 0;
            crypto0InterruptEnable = 0;
            crypto0InterruptFlags = 0;
            crypto1InterruptEnable = 0;
            crypto1InterruptFlags = 0;
        }
        ...
        protected override DoubleWordRegisterCollection BuildRegistersCollection()
        {
            var registerDictionary = new Dictionary<long, DoubleWordRegister>
            {
                {(long)Registers.IpVersion, new DoubleWordRegister(this)
                    .WithValueField(0, 32, FieldMode.Read, valueProviderCallback: _ => 0x00000001U, name: "IPVERSION")
                },
                {(long)Registers.Enable, new DoubleWordRegister(this)
                    .WithFlag(0, out enableField, name: "ENABLE")
                    .WithReservedBits(1, 31)
                },
                {(long)Registers.Control, new DoubleWordRegister(this)
                    .WithReservedBits(0, 32)
                },
                {(long)Registers.Info, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => infoRegister,
                        writeCallback: (_, value) =>
                        {
                            infoRegister = (uint)value;
                        },
                        name: "INFO")
                },
                {(long)Registers.Status, new DoubleWordRegister(this)
                    .WithValueField(0, 32, FieldMode.Read, valueProviderCallback: _ => statusRegister | 0x1U, name: "STATUS")
                },
                {(long)Registers.IrkInterruptEnable, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => irkInterruptEnable,
                        writeCallback: (_, value) =>
                        {
                            irkInterruptEnable = (uint)value;
                            UpdateInterrupts();
                        },
                        name: "IRKINTERRUPTENABLE")
                },
                {(long)Registers.IrkInterruptFlags, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => irkInterruptFlags,
                        writeCallback: (_, value) =>
                        {
                            irkInterruptFlags &= ~(uint)value; // Writing 1 clears the flag
                            UpdateInterrupts();
                        },
                        name: "IRKINTERRUPTFLAGS")
                },
                {(long)Registers.Crypto0InterruptEnable, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => crypto0InterruptEnable,
                        writeCallback: (_, value) =>
                        {
                            crypto0InterruptEnable = (uint)value;
                            UpdateInterrupts();
                        },
                        name: "CRYPTO0INTERRUPTENABLE")
                },
                {(long)Registers.Crypto0InterruptFlags, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => crypto0InterruptFlags,
                        writeCallback: (_, value) =>
                        {
                            crypto0InterruptFlags &= ~(uint)value; // Writing 1 clears the flag
                            UpdateInterrupts();
                        },
                        name: "CRYPTO0INTERRUPTFLAGS")
                },
                {(long)Registers.Crypto1InterruptEnable, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => crypto1InterruptEnable,
                        writeCallback: (_, value) =>
                        {
                            crypto1InterruptEnable = (uint)value;
                            UpdateInterrupts();
                        },
                        name: "CRYPTO1INTERRUPTENABLE")
                },
                {(long)Registers.Crypto1InterruptFlags, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => crypto1InterruptFlags,
                        writeCallback: (_, value) =>
                        {
                            crypto1InterruptFlags &= ~(uint)value; // Writing 1 clears the flag
                            UpdateInterrupts();
                        },
                        name: "CRYPTO1INTERRUPTFLAGS")
                },
                {(long)Registers.RootControl, new DoubleWordRegister(this)
                    .WithValueField(0, 32, 
                        valueProviderCallback: _ => rootControlRegister,
                        writeCallback: (_, value) =>
                        {
                            rootControlRegister = (uint)value;
                        },
                        name: "ROOTCONTROL")
                },
                {(long)Registers.RootBaseAddressMetadata, new DoubleWordRegister(this)
                    .WithReservedBits(0, 2)
                    .WithValueField(2, 30, 
                        valueProviderCallback: _ => rootBaseAddressMetadata,
                        writeCallback: (_, value) =>
                        {
                            rootBaseAddressMetadata = (uint)value;
                        },
                        name: "ROOTBASEADDRMETADATA")
                },
            };
        ...
        protected override void UpdateInterrupts()
        {
            ...
            {
                var pending = ((irkInterruptFlags & irkInterruptEnable) != 0) ||
                             ((crypto0InterruptFlags & crypto0InterruptEnable) != 0) ||
                             ((crypto1InterruptFlags & crypto1InterruptEnable) != 0);
                IRQ.Set(pending);
            });
        }
        ...
        private IFlagRegisterField enableField;
        private uint statusRegister;
        private uint infoRegister;
        private uint rootControlRegister;
        private uint rootBaseAddressMetadata;
        private uint irkInterruptEnable;
        private uint irkInterruptFlags;
        private uint crypto0InterruptEnable;
        private uint crypto0InterruptFlags;
        private uint crypto1InterruptEnable;
        private uint crypto1InterruptFlags;
